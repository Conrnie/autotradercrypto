...
# Optional imports for Solana trading (not needed for Hyperliquid)
try:
    import pandas_ta as ta
    HAS_PANDAS_TA = True
except ImportError:
    print("⚠️  pandas_ta not installed - technical indicator functions will not work (OK for Hyperliquid)")
    HAS_PANDAS_TA = False
    ta = None
...
try:
    ...
    print("⚠️  solders not installed - Solana functions will not work (OK for Hyperliquid)")
    HAS_SOLDERS = False
    solders = None
import shutil
import atexit
...
# Get API keys from environment (BirdEye is optional - not needed for Hyperliquid trading)
BIRDEYE_API_KEY = os.getenv("BIRDEYE_API_KEY")
if not BIRDEYE_API_KEY:
    cprint("⚠️ BIRDEYE_API_KEY not found - Solana/BirdEye functions will not work", "yellow")
    cprint("ℹ️  This is OK if you're only using Hyperliquid", "cyan")
