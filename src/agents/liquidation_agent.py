...
AI_MAX_TOKENS = 0  # Set > 0 to override config.AI_MAX_TOKENS (use config default)
...
class LiquidationAgent(BaseAgent):
    ...
    def _analyze_opportunity(self, current_longs, current_shorts, previous_longs, previous_shorts):
        ...
        try:
            ...
            try:
                ...
                if not response:
                    print("❌ No response object from DeepSeek")
                    return None

                response_text = response.content.strip()
                print(f"📝 DeepSeek response length: {len(response_text)} chars")
    ...
    def run(self):
        """Run one liquidation monitoring cycle and return context"""
        ...
        try:
            ...
            print(f"❌ Error in liquidation monitoring: {str(e)}")
            traceback.print_exc()
            return None

    def run_standalone(self):
        """Run the liquidation monitor continuously (standalone mode)"""
        print("\n🌊 Starting liquidation monitoring in standalone mode...")
