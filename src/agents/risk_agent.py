...
Uses DeepSeek AI via ModelFactory for all risk decisions
"""

# 🛡️ Risk Override Prompt - The Secret Sauce!
RISK_OVERRIDE_PROMPT = """
Je bent een Risk Management AI 🛡️
...
class RiskAgent(BaseAgent):
    def __init__(self):
        ...
        # Use ModelFactory to get DeepSeek model (only model we support now)
        self.model = model_factory.get_model("deepseek")

        if not self.model:
            raise ValueError("🚨 DeepSeek model not available! Check DEEPSEEK_KEY in .env")

        print("🚀 DeepSeek model initialized via ModelFactory!")
    ...
    def get_portfolio_value(self):
        ...
        try:
            print("\n🔍 Portfolio Waarde Calculator Starting...")

            # Check if using Hyperliquid testnet - always use mock capital
            if config.EXCHANGE == 'hyperliquid' and config.HYPERLIQUID_TESTNET:
                print("💱 Hyperliquid Testnet - Using Mock Capital")
                print(f"💰 Mock Capital: ${config.TESTNET_CAPITAL}")
                return float(config.TESTNET_CAPITAL)

            # Check if using Hyperliquid mainnet - get real balance
            elif config.EXCHANGE == 'hyperliquid':
                print("💱 Hyperliquid Mainnet - Fetching Real Balance...")
                ...
                print(f"\n💎 Totale Portfolio Waarde: ${total_value:.2f}")
                return total_value
    ...
    def should_override_limit(self, limit_type):
        ...
        try:
            ...
            # Use DeepSeek via ModelFactory
            print("🚀 Using DeepSeek for analysis...")
            system_prompt = "Je bent een Risk Management AI. Analyseer posities en reageer met OVERRIDE of RESPECT_LIMIT."
            ...
            cprint(f"Using model: DeepSeek ({self.model.model_name})", "white", "on_blue")
            print(response_text)
...
"""
            # Use DeepSeek via ModelFactory
            print("🚀 Using DeepSeek for analysis...")
            system_prompt = "Je bent een Risk Management AI. Analyseer de breach en beslis of posities gesloten moeten worden."
...
def main():
    ...
    while True:
        try:
            ...
            print(f"❌ Error: {str(e)}")
            print("🔧 Controleer de logs en probeer opnieuw!")
